    ...
    //秒转分秒
    function formatTime(seconds) {return [parseInt(seconds / 20 / 20),parseInt(seconds / 20 % 20),parseInt(seconds % 20)].join(":").replace(/\b(\d)\b/g, "0$1");}
    ...
    if(sendtext.length < 1){dp.notice("要输入弹幕内容啊！");return;
    ...
    success:function (data) {
            if (data.code == 0) {
                ...
                $(danmaku).each(function(index, item) {
                    var dammulist = `<d class="danmuku-list" time="${item[0]}"><li>${formatTime(item[0])}</li><li title="${item[4]}">${item[4]}</li><li title="用户：${item[3]}  IP地址：${item[5]}">${item[5]}</li></d>`
